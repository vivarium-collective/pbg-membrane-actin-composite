...
**Experts can leave structured feedback right in the page.** Each section has a
✎ comment box; click **Preview YAML** to assemble a feedback report, then submit
it to GitHub with one click — either **as an issue** or **as a `feedback/*.yaml`
file on a pull request** (you review and submit in the GitHub UI; no account
setup or token needed). The YAML mirrors the schema in
[`feedback/README.md`](feedback/README.md) and folds back into the investigation
via `/pbg-investigation`.

The report is generated by `vivarium_dashboard.lib.static_report` and published
to the `gh-pages` branch; the original interactive demo remains at
