# Next steps for pbg-membrane-actin-composite
...
Composite documents — process-bigraph state trees — live here. Pick one, inspect its wiring (process tree, store paths, emitters), and run it from the **Composite Explorer** sub-page to confirm it produces sensible output before wiring it into an investigation. Composites you write directly into `pbg_membrane_actin_composite/composites/` show up automatically.
...
- **Generate from natural language** — describe what you want; the dashboard writes a request file and prompts you to run `/pbg-viz <name>`, which scaffolds a new Visualization function into `pbg_membrane_actin_composite/visualizations/` and commits it on a stage branch.
